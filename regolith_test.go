...
func TestOpenUnusablePath(t *testing.T) {
	...
	if prefix := ErrUnexpected.Error() + ": "; !strings.HasPrefix(err.Error(), prefix) {
		t.Errorf("expected %q to be wrapped with a detail, got %v", prefix, err)
	}
}
...
// locals on the Go heap.  Removing any one #cgo noescape or #cgo nocallback
// line in regolith.go makes the corresponding local escape and this test
// catch it; so does reintroducing a heap-allocated error detail on the
// success path.
